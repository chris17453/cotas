...
- **`question`** · `f/c/e` · *Required*

  You may use this in two ways. It can be an A type field and the program will display the field as the message. It can also be a numeric value (any appropriate type), and the program will read the appropriate record in the ERRMSG.B file. The value you set must relate to the ERROR_NUM field in the ERRMSG record. For more information please see Chapter 3, Main Menu - Utilities.

- **`default_response`** · `f/c/e` · *Optional*

  The normal default response to this command is .T.. This would be the case if the user pressed just the RETURN or ENTER key without entering a value. If you would like the default response to be N (.F.) then this value must be set to .F.
